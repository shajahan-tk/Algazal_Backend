...
exports.exportBankReportsToExcel = exports.getBankReportStatistics = exports.getBankFinancialSummary = exports.deleteBankReport = exports.updateBankReport = exports.getBankReport = exports.getBankReports = exports.createBankReport = void 0;
...
exports.createBankReport = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const { reportType, reportDate, amount, remarks, 
    // ADIB fields
    category, shop, 
    // Expense fields
    ...
    if (reportType === "adib") {
        if (!category || !shop) {
            throw new apiHandlerHelpers_2.ApiError(400, "Category and shop are required for ADIB reports");
        }
    }
    else if (reportType === "expense") {
        if (!description) {
            throw new apiHandlerHelpers_2.ApiError(400, "Description is required for expense reports");
        }
    }
    // Check references
    if (shop) {
    ...
    if (files.length > 0) {
        ...
        if (!uploadResults.success) {
            throw new apiHandlerHelpers_2.ApiError(500, "Failed to upload report attachments");
        }
        attachments =
    ...
    // Create the report
    const report = await bankModel_1.BankReport.create({
        reportType,
    ...
    res
        .status(201)
        .json(new apiHandlerHelpers_1.ApiResponse(201, report, "Bank report created successfully"));
});
// Get all reports with filters
exports.getBankReports = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    ...
    const total = await bankModel_1.BankReport.countDocuments(filter);
    // Calculate total amount of all matching reports
    const totalAmountResult = await bankModel_1.BankReport.aggregate([
        { $match: filter },
        { $group: { _id: null, totalAmount: { $sum: "$amount" } } },
    ]);
    const totalAmount = totalAmountResult[0]?.totalAmount || 0;
    const reports = await bankModel_1.BankReport.find(filter)
        .skip(skip)
        .limit(limit)
        .sort({ reportDate: -1 })
...
exports.getBankReport = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const { id } = req.params;
    const report = await bankModel_1.BankReport.findById(id)
        .populate("category", "name description")
        .populate("shop", "shopName shopNo")
        .populate("createdBy", "firstName lastName email");
    if (!report) {
        throw new apiHandlerHelpers_2.ApiError(404, "Bank report not found");
    }
    res
        .status(200)
        .json(new apiHandlerHelpers_1.ApiResponse(200, report, "Bank report retrieved successfully"));
});
exports.updateBankReport = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const { id } = req.params;
    ...
    if (files.length > 0) {
        const uploadResults = await (0, uploadConf_1.handleMultipleFileUploads)(files);
        if (!uploadResults.success) {
            throw new apiHandlerHelpers_2.ApiError(500, "Failed to upload new report attachments");
        }
        newAttachments =
            uploadResults.uploadData?.map((file) => ({
                fileName: file.key.split("/").pop() || "attachment",
    ...
    // Update the report
    const updatedReport = await bankModel_1.BankReport.findByIdAndUpdate(id, updatePayload, {
        new: true,
    })
        .populate("category", "name description")
        .populate("shop", "shopName shopNo")
        .populate("createdBy", "firstName lastName email");
    if (!updatedReport) {
        throw new apiHandlerHelpers_2.ApiError(500, "Failed to update bank report");
    }
    res
        .status(200)
        .json(new apiHandlerHelpers_1.ApiResponse(200, updatedReport, "Bank report updated successfully"));
});
// Delete a report
exports.deleteBankReport = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const { id } = req.params;
    const report = await bankModel_1.BankReport.findById(id);
    if (!report) {
        throw new apiHandlerHelpers_2.ApiError(404, "Bank report not found");
    ...
    await bankModel_1.BankReport.findByIdAndDelete(id);
    res
        .status(200)
        .json(new apiHandlerHelpers_1.ApiResponse(200, null, "Bank report deleted successfully"));
});
// Get financial summary
exports.getBankFinancialSummary = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    ...
    const summary = await bankModel_1.BankReport.aggregate(pipeline);
    res
        .status(200)
        .json(new apiHandlerHelpers_1.ApiResponse(200, summary, "Bank financial summary retrieved successfully"));
});
// Get report statistics
exports.getBankReportStatistics = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    ...
    const stats = await bankModel_1.BankReport.aggregate([
        { $match: match },
        {
            $facet: {
    ...
    res
        .status(200)
        .json(new apiHandlerHelpers_1.ApiResponse(200, stats[0], "Bank report statistics retrieved successfully"));
});
exports.exportBankReportsToExcel = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const filter = {};
    // Report type filter
    if (req.query.reportType) {
        filter.reportType = req.query.reportType;
    ...
    res.setHeader("Content-Disposition", `attachment; filename=bank_reports_export_${new Date().toISOString().split("T")[0]}.xlsx`);
