...
exports.getPayslipData = exports.generatePayslipPDF = exports.getPayroll = exports.exportPayrollsToExcel = exports.deletePayroll = exports.updatePayroll = exports.createPayroll = exports.getPayrolls = exports.calculatePreviousMonthOvertimeAmount = void 0;
...
const calculatePreviousMonthOvertimeAmount = async (userId, basicSalary) => {
    try {
        const now = new Date();
...
exports.calculatePreviousMonthOvertimeAmount = calculatePreviousMonthOvertimeAmount;
...
const getAttendanceDetails = async (userId, period) => {
    try {
        // Validate inputs
        if (!userId || !period) {
            return {
        ...
        const attendances = await attendanceModel_1.Attendance.find(filter)
...
const getAttendanceDetailsByMonthYear = async (userId, month, year) => {
    try {
        // Validate inputs
...
exports.getPayrolls = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    ...
    if (startDate && endDate) {
        ...
        if (isNaN(start.getTime()) || isNaN(end.getTime())) {
            throw new apiHandlerHelpers_2.ApiError(400, "Invalid date format in startDate or endDate");
        }
        filter.createdAt = {
            $gte: start,
            $lte: end
    ...
    const enhancedPayrolls = await Promise.all(payrolls.map(async (payroll) => {
        ...
        const attendanceDetails = await getAttendanceDetailsByMonthYear(payroll.employee._id, getMonthNumber(payroll.period.split('-')[0]), parseInt(payroll.period.split('-')[1]));
...
exports.createPayroll = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    ...
    if (existingPayroll) {
        throw new apiHandlerHelpers_2.ApiError(400, `Payroll already exists for this employee for period ${period}`);
    }
    ...
    const overtimeData = await (0, exports.calculatePreviousMonthOvertimeAmount)(employee, basicSalary);
    ...
    res.status(201).json(new apiHandlerHelpers_1.ApiResponse(201, payroll, "Payroll created successfully for previous month"));
});
...
exports.updatePayroll = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    ...
    if (updateData.transport !== undefined)
        updateData.transport = Number(updateData.transport) || 0;
    if (updateData.specialOT !== undefined)
        updateData.specialOT = Number(updateData.specialOT) || 0;
    if (updateData.medical !== undefined)
        updateData.medical = Number(updateData.medical) || 0;
    if (updateData.bonus !== undefined)
        updateData.bonus = Number(updateData.bonus) || 0;
    if (updateData.mess !== undefined)
        updateData.mess = Number(updateData.mess) || 0;
    if (updateData.salaryAdvance !== undefined)
        updateData.salaryAdvance = Number(updateData.salaryAdvance) || 0;
    if (updateData.loanDeduction !== undefined)
        updateData.loanDeduction = Number(updateData.loanDeduction) || 0;
    if (updateData.fineAmount !== undefined)
        updateData.fineAmount = Number(updateData.fineAmount) || 0;
    if (updateData.visaDeduction !== undefined)
        updateData.visaDeduction = Number(updateData.visaDeduction) || 0;
    ...
    if (!updatedPayroll) {
        throw new apiHandlerHelpers_2.ApiError(404, "Payroll not found after update");
    }
...
exports.generatePayslipPDF = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const { id } = req.params;
    const payroll = await payrollModel_1.Payroll.findById(id).populate({
        path: 'employee',
        select: 'firstName lastName role emiratesId passportNumber iBANNumber'
    });
    if (!payroll) {
        throw new apiHandlerHelpers_2.ApiError(404, "Payroll record not found");
    }
    if (!payroll.employee || typeof payroll.employee !== 'object') {
        throw new apiHandlerHelpers_2.ApiError(500, "Employee data not properly populated");
    }
    ...
    const visaExpense = await visaExpenseModel_1.VisaExpense.findOne({
        employee: payroll.employee._id
    }).lean();
    const basicSalary = employeeExpense?.basicSalary || 0;
...
exports.getPayslipData = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const { id } = req.params;
    const payroll = await payrollModel_1.Payroll.findById(id).populate({
        path: 'employee',
        select: 'firstName lastName role emiratesId passportNumber iBANNumber'
    ...
    res.status(200).json(new apiHandlerHelpers_1.ApiResponse(200, payslipData, "Payslip data retrieved successfully"));
});
