...
exports.exportVisaExpensesToExcel = exports.deleteVisaExpense = exports.updateVisaExpense = exports.getVisaExpense = exports.getVisaExpenseStats = exports.getVisaExpenses = exports.createVisaExpense = void 0;
...
exports.getVisaExpenses = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    ...
    try {
        ...
        if (req.query.employee) {
            if (!mongoose_1.Types.ObjectId.isValid(req.query.employee)) {
                throw new apiHandlerHelpers_2.ApiError(400, "Invalid employee ID format");
            }
            filter.employee = new mongoose_1.Types.ObjectId(req.query.employee);
        }
        ...
        if (req.query.startDate && req.query.endDate) {
            ...
            if (isNaN(startDate.getTime()) || isNaN(endDate.getTime())) {
                throw new apiHandlerHelpers_2.ApiError(400, "Invalid date format. Use YYYY-MM-DD format");
            }
            if (startDate > endDate) {
                throw new apiHandlerHelpers_2.ApiError(400, "Start date cannot be later than end date");
            }
        ...
        // Month/Year filter (only if date range is not provided)
            ...
            if (isNaN(year) || isNaN(month) || month < 1 || month > 12) {
                throw new apiHandlerHelpers_2.ApiError(400, "Invalid month or year. Month should be 1-12");
            }
        ...
        // Year only filter
            ...
            const year = parseInt(req.query.year);
            if (isNaN(year)) {
                throw new apiHandlerHelpers_2.ApiError(400, "Invalid year format");
            }
            filter.createdAt = {
        ...
        if (req.query.minTotal || req.query.maxTotal) {
            ...
            if (req.query.minTotal) {
                ...
                if (isNaN(minTotal) || minTotal < 0) {
                    throw new apiHandlerHelpers_2.ApiError(400, "Invalid minimum total amount");
                }
            ...
            if (req.query.maxTotal) {
                ...
                if (isNaN(maxTotal) || maxTotal < 0) {
                    throw new apiHandlerHelpers_2.ApiError(400, "Invalid maximum total amount");
                }
            ...
            if (req.query.minTotal && req.query.maxTotal) {
                ...
                if (minTotal > maxTotal) {
                    throw new apiHandlerHelpers_2.ApiError(400, "Minimum total cannot be greater than maximum total");
                }
            }
        }
        ...
        const aggregatedData = await visaExpenseModel_1.VisaExpense.aggregate([
            { $match: filter },
            {
                $group: {
                    _id: null,
        ...
        res.status(200).json(new apiHandlerHelpers_1.ApiResponse(200, responseData, `${total} visa expense${total === 1 ? '' : 's'} retrieved successfully`));
    }
    catch (error) {
        if (error instanceof apiHandlerHelpers_2.ApiError) {
            throw error;
        }
        // Handle MongoDB errors
        if (error instanceof Error) {
            if (error.message.includes('Cast to ObjectId failed')) {
                throw new apiHandlerHelpers_2.ApiError(400, "Invalid ID format provided");
            }
            throw new apiHandlerHelpers_2.ApiError(500, `Database error: ${error.message}`);
        }
        throw new apiHandlerHelpers_2.ApiError(500, "An unexpected error occurred while fetching visa expenses");
    }
});
// Additional helper function to get visa expense statistics
exports.getVisaExpenseStats = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    try {
        const stats = await visaExpenseModel_1.VisaExpense.aggregate([
            {
                $group: {
                    _id: null,
        ...
        const monthlyStats = await visaExpenseModel_1.VisaExpense.aggregate([
            {
                $match: {
                    createdAt: {
        ...
        res.status(200).json(new apiHandlerHelpers_1.ApiResponse(200, responseData, "Visa expense statistics retrieved successfully"));
    }
    catch (error) {
        throw new apiHandlerHelpers_2.ApiError(500, "Error retrieving visa expense statistics");
    }
});
...
exports.exportVisaExpensesToExcel = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    try {
        ...
        const visaExpenses = await visaExpenseModel_1.VisaExpense.find(filter)
            .sort({ createdAt: -1 })
            .populate("employee", "firstName lastName phoneNumbers role department country bankNumber")
            .populate("createdBy", "firstName lastName")
            .lean();
        if (visaExpenses.length === 0) {
            throw new apiHandlerHelpers_2.ApiError(404, "No visa expenses found matching the criteria");
        }
        ...
        if (error instanceof Error) {
            ...
            throw new apiHandlerHelpers_2.ApiError(500, `Export error: ${error.message}`);
        }
        throw new apiHandlerHelpers_2.ApiError(500, "An unexpected error occurred during export");
    }
});
