...
const migrateAttendance = async () => {
    try {
        ...
        for await (const group of cursor) {
            ...
            // Checking for migration status: if type is 'project' and projects array is empty but project field exists.
                ...
                if (record.projects && record.projects.length > 0) {
                    for (const p of record.projects) {
                        if (p.project) {
                            ...
                            // Overwrite or add? Let's assume array data is newer/better if it exists
                            projectsMap.set(pId, {
                                project: p.project,
            ...
            if (isPaidLeave) {
                ...
                // Ensure projects reflect 0 hours if any
                projects.forEach(p => {
                    p.workingHours = 0;
                    p.present = false;
                });
